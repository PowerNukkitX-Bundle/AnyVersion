...
public class StoneBlockSlab2Transformer extends ItemDataTransformer {
    ...
    public ItemData transform(ItemData original) {
        ...
        int runtimeId = original.getDefinition().getIdentifier().startsWith("minecraft:double_") ? 181 : 182;
        NbtMap map = NbtMap.fromMap((Map<String, Object>) definition.getState().get("states"));
        int damage = map.isEmpty() || map.getString("stone_slab_type_2").isEmpty() ? 0 : StoneSlabType2.valueOf(map.getString("stone_slab_type_2").toUpperCase()).ordinal();
