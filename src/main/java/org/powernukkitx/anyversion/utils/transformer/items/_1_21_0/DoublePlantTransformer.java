...
public class DoublePlantTransformer extends ItemDataTransformer {
    ...
    public ItemData transform(ItemData original) {
        ...
        int damage = map.isEmpty() || map.getString("double_plant_type").isEmpty() ? 0 : DoublePlantType.valueOf(map.getString("double_plant_type").toUpperCase()).ordinal();
        SimpleItemDefinition itemDefinition = new SimpleItemDefinition(originalItemDefinition.getIdentifier(), Registries.ITEM_RUNTIMEID.get(BlockID.SUNFLOWER), originalItemDefinition.getVersion(), originalItemDefinition.isComponentBased(), originalItemDefinition.getComponentData());
