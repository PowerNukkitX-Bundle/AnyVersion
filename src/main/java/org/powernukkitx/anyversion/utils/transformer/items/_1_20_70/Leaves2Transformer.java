...
public class Leaves2Transformer extends ItemDataTransformer {
    ...
    public ItemData transform(ItemData original) {
        ...
        int damage = map.isEmpty() || map.getString("new_leaf_type").isEmpty() ? 0 : NewLeafType.valueOf(map.getString("new_leaf_type").toUpperCase()).ordinal();
        SimpleItemDefinition itemDefinition = new SimpleItemDefinition(originalItemDefinition.getIdentifier(), Registries.ITEM_RUNTIMEID.get(BlockID.ACACIA_LEAVES), originalItemDefinition.getVersion(), originalItemDefinition.isComponentBased(), originalItemDefinition.getComponentData());
